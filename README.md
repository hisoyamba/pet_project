...
1. Airflow по расписанию забирает CSV с данными о землетрясениях из API USGS.
2. Сырой файл складывается в S3-бакет (MinIO) — это «золотой» слой Data Lake.
3. DuckDB читает CSV из S3 и загружает данные в схему `ods` PostgreSQL.
4. На основе ODS формируются витрины в схеме `dm` (количество и средняя магнитуда по дням).
5. Metabase подключается к PostgreSQL и строит дашборды поверх витрин.
